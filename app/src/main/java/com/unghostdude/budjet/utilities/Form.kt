...
sealed class Validators {
    class Required(errorMessage: String = "Field is required") : FormValidator(errorMessage) {
        override fun isValid(value: String): ValidationResult {
            return if (value.isNotEmpty()) ValidationResult(true, "") else ValidationResult(false, errorMessage)
        }
    }

    class MinLength(private val min: Int, errorMessage: String = "Field length must be at least $min") : FormValidator(errorMessage) {
        override fun isValid(value: String): ValidationResult {
            return if (value.length >= min) ValidationResult(true, "") else ValidationResult(false, errorMessage)
        }
    }

    class MaxLength(private val max: Int, errorMessage: String = "") : FormValidator(errorMessage) {
        override fun isValid(value: String): ValidationResult {
            return if (value.length <= max) ValidationResult(true, "") else ValidationResult(false, errorMessage)
        }
    }

    class In(private val range: List<String>, errorMessage: String = "") : FormValidator(errorMessage) {
        override fun isValid(value: String): ValidationResult {
            return if (range.contains(value)) ValidationResult(true, "") else ValidationResult(false, errorMessage)
        }
    }
}